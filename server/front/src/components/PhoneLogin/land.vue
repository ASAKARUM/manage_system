...
export default {
  ...
  data() {
    /**
     * @prop {String} url 存放图片地址
     * @prop {String} active 存放页面展示的ID
     * @prop {Boolean} show 判断展示的页面功能
     * @prop {Boolean} showx 判断展示的页面类型
     * @prop {String} count 存放获取验证码按钮的时间
     * @prop {Object} time 存放点击获取验证码按钮的剩余时间状态
     * @prop {Object} ruleForm2 存放用户注册时的手机号、孩子姓名和验证码
     * @prop {Object} ruleForm 存放用户通过手机号和验证码方式登录的手机号和验证码
     * @prop {Object} ruleForm3 存放用户通过手机号密码方式登录的手机号和密码
     * @prop {Array} images 存放图片信息
     */
    return {
