...
class OtherResource(models.Model):
    ...
    :字段解释:
    + customer: 家长 类型为外键, 对应Customer类
    + resource: 流量来源信息 类型为CharField, 最大长度为200
    + resource_date: 获取来源的日期, 类型为DateField
    '''
...
class AuditionRecord(models.Model):
    ...
    :字段解释:
    + customer: 家长 类型为外键, 对应Customer类
    + section: 课节 类型为外键,  对应Section类
    + is_signedup: 是否报名 类型为BooleanField, 默认为False即代表没报名
    + date: 试听的日期及时间, 类型为DateTimeField, 默认为记录生成时
    + info: 试听情况反馈信息 类型为TextField
    '''
...
class SellRecord(models.Model):
    ...
    :字段解释:
    + customer: 家长 类型为外键, 对应Customer类
    + seller: 地推人员 类型为外键,  对应SellerInfo类
    + money: 注册金额 类型为FloatField
    + date: 注册的日期及时间, 类型为DateTimeField, 默认为记录生成时
    + info: 试听情况反馈信息 类型为TextField
    '''
    customer = models.ForeignKey(
...
class RefundRecord(models.Model):
    ...
    :字段解释:
    + customer: 家长 类型为外键, 对应Customer类
    + course: 对应课程, 类型为外键, 对应Courses类
    + refund: 退款金额 类型为FloatField
    + cc: 家长对应的课程顾问 类型为外键, 对应User类
    + date: 申请退款的日期及时间 类型为DateTimeField, 默认为记录生成时
    + deal_date: 课程顾问处理退款信息的时间 类型为DateTimeField
    + is_passed: 课程顾问是否通过 类型为BooleanField, 默认为False即代表没通过
    + is_paid: 是否退款 类型为BooleanField, 默认为False即代表没退款
    + reason: 退款原因 类型为TextField
    '''
    customer = models.ForeignKey(
...
class CcRecord(models.Model):
    ...
    :字段解释:
    + customer: 家长 类型为外键, 对应Customer类
    + cc: 课程顾问 类型为外键, 对应User类
    + money: 正式课报名金额 类型为FloatField
    + date: 正式课报名的日期及时间 类型为DateTimeField, 默认为记录生成时
    + info: 课程需求 类型为TextField
    + is_paid: 是否付款 类型为BooleanField, 默认为False即代表没付
    + trade_no: 订单号 类型为CharField, 最大长度为20
    + course: 所选课程 类型为外键 对应Courses类
    '''
    customer = models.ForeignKey(
...
class OperationLogs(models.Model):
    ...
    :字段解释:
    + TYPE_CHOICES: 类型选择项 0: 退款记录, 1: 删除员工, 2: 删除用户, 3: 增添员工
    + operator: 操作者 类型为外键, 对应User类
    + type: 操作类型 类型为PositiveSmallIntegerField
    + username: 被操作对象名 类型为CharField, 最大长度为20
    + date: 操作执行的日期及时间 类型为DateTimeField, 默认为生成记录时
    + info: 操作详细信息 类型为TextField, 最大长度为500
    '''
    TYPE_CHOICES = (
        (0, 'refund'),
        (1, 'del_user'),
